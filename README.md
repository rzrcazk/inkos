...
**dev 分支 — Studio 配置体系升级** — LLM 配置体系重构：移除 legacy env 模式，统一为 Studio 服务配置 + `.inkos/secrets.json` 管理。新增服务供应商管理页面（可视化配置、测试连接、模型选择）、模型路由 UI（per-agent 模型分配 + task routes 配置）、模型能力页面（各模型能力矩阵）。市场雷达新增服务/模型选择器；智能路由算法重构；LLM 配置校验增强。
...
1. 选择服务商，例如 Google Gemini、Moonshot、MiniMax、智谱、百炼、Anthropic 或自定义端点。
...
CLI 运行时复用 Studio 配置好的服务和密钥，可通过 CLI 参数一次性覆盖（`--service`、`--model`、`--api-key-env` 等）。
...
`doctor` 会显示当前 service/model/API Key 来源，并尝试 API 连通性。
...
- **统一服务配置**：Studio 和 CLI 共享同一套服务配置体系，通过 Studio 服务页可视化管理。API Key 存储在 `.inkos/secrets.json`，不会写进 `inkos.json`。
- **Provider bank 能力表**：内置 Google Gemini、Moonshot、MiniMax、智谱、百炼、DeepSeek、硅基流动、PPIO、OpenRouter、Ollama、CodingPlan、Anthropic 等服务的 baseUrl、协议、模型和兼容策略。
- **模型路由 UI**：Studio 新增模型路由页面，支持 per-agent 模型分配和 task routes 配置，未配置的 agent 自动回退默认服务。
- **模型能力矩阵**：Studio 新增模型能力页面，可视化展示各模型的支持能力。
- **模型归属校验**：`--service google --model kimi-k2.5` 这类错配会直接报错，避免把请求发到错误服务商。未配置的 modelOverrides 会抛出明确错误而非静默回退。
- **Google Gemini 兼容修复**：AI Studio API Key 可直接用于 Gemini OpenAI-compatible endpoint，InkOS 会自动禁用 Google 不支持的 OpenAI `store` 参数。
- **Anthropic API 格式支持**：服务配置支持 Anthropic 原生 API 格式。
- **MiniMax transport 探测**：MiniMax / MiniMax CodingPlan 自动使用可工作的 transport，规避流式 usage 正常但正文为空的问题。
- **市场雷达增强**：Radar 扫描支持选择服务和模型，避免竞态条件下的模型回退问题。
...
| `inkos config set-global` | 设置全局 LLM 配置（`~/.inkos/secrets.json`），所有项目共享 |
| `inkos config show-global` | 查看全局配置 |
| `inkos config set/show` | 查看/更新项目配置 |
| `inkos config set-model <agent> <model>` | 为指定 agent 设置模型覆盖（`--base-url`、`--provider` 支持多 Provider 路由） |
| `inkos config remove-model <agent>` | 移除 agent 模型覆盖（回退到默认） |
| `inkos config show-models` | 查看当前模型路由 |
| `inkos doctor` | 诊断配置问题（显示 service/model 来源、API 连通性和提供商兼容性提示） |
